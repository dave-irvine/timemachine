...
Repository.prototype.applyCommitFilter = function (filter) {
	...
	if (filter.inDateRanges) {
		repository.commits = filterCommitsInDateRanges(repository.commits, filter.inDateRanges).slice(0);

		if (repository.commits.length < 1) {
			throw new Error("TimeMachine :: Repo does not contain any commits in the specified date ranges: " + filter.inDateRanges);
		}
	}
};
...
filterCommitsInDateRanges = function (commits, ranges) {
	var repository = this,
		commitsInDateRanges;

	commitsInDateRanges = commits.filter(function (commit) {
		return commit.inDateRanges(ranges);
	});

	return commitsInDateRanges;
};
